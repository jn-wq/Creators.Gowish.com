...
export const Route = createFileRoute("/learning-bank")({
  head: () => ({
    meta: [
      { title: "Learning Bank — GoWish Creator Universe" },
      { name: "description", content: "Real case studies from creators earning on GoWish. Tactics, numbers, and frameworks you can copy." },
      { property: "og:title", content: "Learning Bank — GoWish Creator" },
      { property: "og:description", content: "Case studies from creators earning on GoWish." },
    ],
  }),
      ...
      <main>
        <section className="relative overflow-hidden border-b border-line">
          ...
          <div className="relative mx-auto max-w-[1180px] px-6 pt-20 pb-20">
            <p className="eyebrow flex items-center gap-2"><BookOpen className="h-4 w-4" /> Learning Bank</p>
            <h1 className="display mt-5 text-[clamp(2.4rem,6vw,4.8rem)] max-w-[18ch]">
              Case studies <span className="serif text-accent">from creators</span> who actually earn.
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <label className="flex items-center gap-2 rounded-full border border-line bg-bg px-4 py-2.5 w-full max-w-md">
                ...
                <input
                  type="search"
                  placeholder="Search case studies, creators, categories…"
                  className="flex-1 bg-transparent outline-none text-[15px] placeholder:text-ink-3"
                />
              </label>
              <span className="label-kpi">{caseStudies.length} studies · {categories.length} categories</span>
            </div>
        ...
        <section className="mx-auto max-w-[1180px] px-6 pt-20">
          <p className="eyebrow mb-6">Featured study</p>
          {(() => {
            ...
            return (
              ...
              >
                <div className="bg-cream-soft p-10 md:p-14 relative">
                  <span className="label-kpi !text-accent-ink">{f.category}</span>
                  <h2 className="title-l mt-4 text-[clamp(1.8rem,3.6vw,2.8rem)] max-w-[20ch]">
                    {f.title}
                  </h2>
                  <p className="mt-5 text-[17px] text-ink-2 max-w-[52ch] leading-[1.5]">{f.excerpt}</p>
                  <div className="mt-8 flex items-center gap-4">
                    <Avatar name={f.creator} />
                    <div>
                      <div className="font-semibold text-[15px]">{f.creator}</div>
                      <div className="text-[13px] text-ink-3">{f.handle} · {f.readTime}</div>
                    </div>
                  </div>
                </div>
                <div className="bg-bg p-10 md:p-14 flex flex-col justify-between gap-8">
                  ...
                  <span className="inline-flex items-center gap-2 text-accent-ink font-semibold group-hover:gap-3 transition-all">
                    Read the playbook <ArrowUpRight className="h-4 w-4" />
                  </span>
                </div>
              </Link>
            );
        ...
        <section className="mx-auto max-w-[1180px] px-6 pt-16 pb-8">
          <p className="eyebrow mb-6">All case studies</p>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {caseStudies.slice(1).map((c) => (
              ...
              >
                ...
                <h3 className="text-[22px] font-semibold tracking-tight leading-[1.15]">{c.title}</h3>
                <p className="text-[15px] text-ink-2 leading-[1.5] line-clamp-3">{c.excerpt}</p>
